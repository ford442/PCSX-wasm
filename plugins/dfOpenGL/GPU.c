...
long CALLBACK GPUinit()
{
	...
	printf("readconfig\n");
	readconfig();
	printf("x,y %d %d\n",gpuConfig.windowX, gpuConfig.windowY);
	// mapping the VRAM
...
long CALLBACK GPUopen(unsigned long * disp,char * CapText,char * CfgFile)
{
	...
	printf("defaul screen\n");
	
	XF86VidModeQueryVersion(GLWin.dpy, &vidModeMajorVersion,
							&vidModeMinorVersion);
	printf("query\n");							
	XF86VidModeGetAllModeLines(GLWin.dpy, GLWin.screen, &modeNum, &modes);
	printf("get alll mode\n");							
	...
	{
		printf("create window %d %p\n", vi->depth, GLWin.dpy);
		printf("%lu %d\n", RootWindow(GLWin.dpy, vi->screen), vi->depth);
		printf("%d %d\n",gpuConfig.windowX, gpuConfig.windowY);
		GLWin.win = XCreateWindow(GLWin.dpy, RootWindow(GLWin.dpy, vi->screen),
								  0, 0, gpuConfig.windowX, gpuConfig.windowY, 0, vi->depth, InputOutput, vi->visual,
								  CWBorderPixel | CWColormap | CWEventMask, &GLWin.attr);
		printf("create window done %lx\n", GLWin.win);
