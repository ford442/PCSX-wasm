...
*/
	...
	printf("len=%d\nconst unsigned char font_8140[]={",len);
	for(int i=0;i<len;i++){
		printf("0x%02x,", *(unsigned char *)(psxR+0x66000+i));
		if(i%16==0) printf("\n");
	}
	printf("}\n");
	len = 0x80000 - 0x69d68;
	uncompress((Bytef *)(psxR + 0x69d68), &len, font_889f, sizeof(font_889f));
	printf("len=%d\nconst unsigned char font_889f[]={",len);
	for(int i=0;i<len;i++){
		printf("0x%02x,", *(unsigned char *)(psxR+0x69d68+i));
		if(i%16==0) printf("\n");
	}	
	printf("}\n");
	fflush(stdout);
	#else
	memcpy((Bytef *)(psxR + 0x66000), font_8140, sizeof(font_8140));
	memcpy((Bytef *)(psxR + 0x69d68), font_889f, sizeof(font_8140));
	#endif
	// memory size 2 MB
